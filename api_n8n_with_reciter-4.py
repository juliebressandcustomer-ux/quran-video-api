...
DEFAULT_CONFIG = {
    ...
    # 🔄 TRANSITIONS ENTRE LOOPS (NOUVEAU - CORRIGÉ)
    "loop_transition": True,  # Transition fluide entre répétitions du background
    "loop_transition_duration": 1.0,  # Durée du crossfade (secondes)
...
def generate_video(background_video, audio_file, ass_file, output_video, config):
    """Génère la vidéo finale avec transitions fluides entre loops"""
    ...
    if video_duration < audio_duration:
        ...
        # 🎬 NOUVELLE APPROCHE : xfade entre les répétitions
        # On split la vidéo input et on fait des xfade entre chaque partie
        
        loop_transition = config.get('loop_transition', True)
        transition_dur = config.get('loop_transition_duration', 1.0)
        
        if loop_transition and loops_needed > 1 and loops_needed <= 10:
            # ✨ AVEC TRANSITIONS - VERSION CORRIGÉE
            print(f"✨ Crossfade entre loops: {transition_dur}s")
            
            # Stratégie: Créer plusieurs inputs identiques et les xfade ensemble
            # [0:v] = input 1, [0:v] = input 2 (même source), etc.
            # puis xfade(offset=video_duration-transition_dur)
            
            # Construire le filtre complexe avec xfade
            filter_parts = []
            
            # Étape 1: Scale et pad chaque input
            ...
            # Étape 2: Xfade entre chaque paire
            current = "v0"
            for i in range(1, loops_needed):
                offset = (i * video_duration) - transition_dur
                if i == 1:
                    filter_parts.append(
                        f"[{current}][v{i}]xfade=transition=fade:duration={transition_dur}:offset={offset}[vx{i}]"
                    )
                else:
                    filter_parts.append(
                        f"[vx{i-1}][v{i}]xfade=transition=fade:duration={transition_dur}:offset={offset}[vx{i}]"
                    )
                current = f"vx{i}"
            
            # Étape 3: Ajouter les sous-titres
            final_output = f"vx{loops_needed-1}" if loops_needed > 1 else "v0"
            filter_parts.append(f"[{final_output}]ass={ass_file}:fontsdir=[v]")
            
            video_filter = ";".join(filter_parts)
            
            cmd = [
                "ffmpeg", "-stream_loop", str(loops_needed - 1), "-i", background_video, "-i", audio_file,
            ...
            # SANS TRANSITIONS ou trop de loops
            if loops_needed > 10:
                print(f"⚠️  Trop de loops ({loops_needed}), transitions désactivées")
            
            video_filter = f"[0:v]loop={loops_needed}:size=32767:start=0,scale={width}:{height}:force_original_aspect_ratio=decrease,pad={width}:{height}:(ow-iw)/2:(oh-ih)/2,ass={ass_file}:fontsdir=[v]"
