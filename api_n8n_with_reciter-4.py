...
DEFAULT_CONFIG = {
    ...
    "min_segments": 1,
    "max_segments": 50,
    "crf": 28,  # Réduit de 23→28 pour moins de RAM (qualité légèrement inférieure)
    "preset": "ultrafast",  # Réduit de fast→ultrafast pour moins de RAM
    "audio_bitrate": "128k",  # Réduit de 192k→128k pour économiser RAM
    "clean_text": True,
...
def generate_ass(text, audio_path, output_ass, config):
    ...
    # Adapter la résolution pour PlayRes (OPTIMISÉ RAILWAY - Résolutions réduites)
    resolution = config.get('resolution', '1080p')
    if resolution == 'vertical':
        play_res_x = 720  # Réduit de 1080 pour économiser RAM
        play_res_y = 1280  # Réduit de 1920 pour économiser RAM
    elif resolution == 'square':
        play_res_x = 720  # Réduit de 1080
        play_res_y = 720  # Réduit de 1080
    elif resolution == '4k':
        play_res_x = 1920  # Réduit de 3840 (4K trop lourd pour Railway)
        play_res_y = 1080  # Réduit de 2160
...
def generate_video(background_video, audio_file, ass_file, output_video, config):
    ...
    resolutions = {
        ...
        'vertical': {'width': 720, 'height': 1280, 'name': 'Vertical (9:16 TikTok/Stories)'},  # Réduit de 1080x1920
        'square': {'width': 720, 'height': 720, 'name': 'Carré (1:1 Instagram)'},  # Réduit de 1080x1080
    ...
    try:
        # FFmpeg en mode silencieux avec optimisations RAM pour Railway
        memory_opts = [
            "-max_muxing_queue_size", "512",  # Réduit buffer
            "-bufsize", "1M",                  # Buffer plus petit
            "-maxrate", "2M",                  # Limite bitrate
            "-threads", "1"                    # 1 thread seulement pour économiser RAM
        ]
...
def list_fonts():
    """Liste toutes les polices arabes disponibles"""
    try:
        ...
        # Extraire les noms de polices uniques
        fonts = []
        seen = set()
        for line in result.stdout.strip().split('\n'):
            if line and line not in seen:
                # Nettoyer le nom (enlever les variantes après la virgule)
