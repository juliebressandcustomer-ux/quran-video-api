...
def generate_video(background_video, audio_file, ass_file, output_video, config):
    ...
    try:
        ...
        subprocess.run(cmd, check=True, capture_output=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
        return True
    except subprocess.CalledProcessError as e:
        print(f"❌ Erreur ffmpeg: {e.stderr.decode() if e.stderr else 'Unknown error'}")
        return False
